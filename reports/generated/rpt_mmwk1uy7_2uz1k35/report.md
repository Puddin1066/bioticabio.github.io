...
**Report parameters (input):** drug: CHEMBL103; diseases: dry eye disease; targets: TRPV1, TRPM8, SCN9A, CHRM3; audience: investor; data mode: live.

**Query structure:** Open Targets GraphQL operations used: drug summary (ChEMBL ID), disease-dry-eye (EFO IDs), target-summary (Ensembl IDs).

Report generated from the **Open Targets Platform GraphQL API**. Entity resolution uses an **ontology translation table** (config/ontology-translation.json): dry eye → EFO proxies (EFO_0000699; Sjogren syndrome), trigeminal pathway → gene proxies (TRPV1, TRPM8, SCN9A, CHRM3). Resolved IDs used in queries: drug (ChEMBL), disease (EFO: EFO_0000699), targets (Ensembl: ENSG00000196689, ENSG00000144481, ENSG00000169432, ENSG00000133019).

**Re-run:** Submit the same parameters via the Report API (`POST /reports`).
