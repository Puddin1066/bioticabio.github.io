...
export interface MapIdsHit {
  ...
  entity?: string;
  score?: number;
  object?: { id: string; name?: string; approvedSymbol?: string; approvedName?: string };
}
...
export interface IOpenTargetsProvider {
  ...
  searchEntities(queryString: string, entityNames: string[], page?: { index: number; size: number }): Promise<SearchResult>;
