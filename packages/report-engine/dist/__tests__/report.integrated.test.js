...
export function runQualityGates() {
    ...
    for (const phrase of PROVENANCE_MUST_CONTAIN) {
        assert(provenanceSection.body.includes(phrase), `Provenance must contain "${phrase}"`);
    }
    assert(!provenanceSection.body.toLowerCase().includes('could not be resolved'), 'Provenance must not contain pipeline-failure tone');
    ...
    assert(mockedMarkdown.includes(MOCK_LABEL), 'Output built from mocked fixture must contain "[MOCK]" label');
    console.log('Quality gates passed.');
}
runQualityGates();
//# sourceMappingURL=report.integrated.test.js.map