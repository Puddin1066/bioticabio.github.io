 ...
 */
export async function resolveDisease(provider, input, translation) {
    ...
    if (trimmed.startsWith(EFO_PREFIX.toLowerCase())) {
        ...
        if (!disease)
            return [];
        const id = disease.id ?? efoId;
        const label = disease.name ?? id;
        return [
            {
                input: raw,
                resolvedId: id,
        ...
        for (const h of m.hits ?? []) {
            if (h.id && (h.entity === 'disease' || (h.object && !('approvedSymbol' in h.object)))) {
                if (hitIds.has(h.id))
                    continue;
                hitIds.add(h.id);
                const label = h.object?.name ?? h.name ?? h.id;
                results.push({
                    input: raw,
                    resolvedId: h.id,
                    label: String(label),
 ...
 */
export async function resolveDiseases(provider, inputs, translation) {
    const seen = new Set();
    const results = [];
...
//# sourceMappingURL=resolveDisease.js.map